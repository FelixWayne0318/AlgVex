  ...
  update-hashes:
    ...
    steps:
      ...
      - name: Install dependencies
        run: pip install PyYAML==6.0.1  # 锁定版本
...
关键约束:
- PYTHONHASHSEED 必须在进程启动前设置 (Docker/shell/cron)
...
def setup_deterministic_env(seed: int = 42, num_threads: int = 1, strict: bool = True):
    ...
    if current_hashseed is None:
        issues.append("PYTHONHASHSEED 未在启动前设置! 请在 Docker/shell/cron 中设置")
...
| 幂等/状态机（重试/重放/断点续跑） | **部分满足（需要补证据）** | 断线重连/重复事件导致重复下单、重复记账、重复信号 | S8 唯一入口 + S9 Canonical Hash + S10 Replay确定性 | 明确 **Order/Fills 事件幂等键**（client_order_id + exchange_order_id + event_seq）；补“重放/重复事件”测试与演练脚本 |
...
| 机制 | 物理隔离（进程/容器/权限）、密钥只在执行侧可见、只读数据出口 |
...
### 6.3 HummingbotBridge

```python
from algvex.core.execution import HummingbotBridge, Signal
...
signal = Signal(symbol="BTCUSDT", score=0.75, timestamp=datetime.now())
order = bridge.signal_to_order(signal=signal, capital=10000, risk_per_trade=0.02, leverage=3)
...
> ✅ **MVP包含** - Docker Compose单机部署即可，Kubernetes延后。
...
services:
  ...
  beat:
    ...
    depends_on:
      - redis
    command: celery -A api.celery beat -l info
...
# 6. 访问
# 前端: https://algvex.com
# API文档: https://algvex.com/api/docs
```
...
11. **3轮迭代交付** - Iter-1契约 → Iter-2对齐 → Iter-3快照
