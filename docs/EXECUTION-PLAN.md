...
| 依赖 | Qlib 要求 | Hummingbot 要求 | 冲突 |
|------|-----------|-----------------|------|
| numpy | `<2.0.0` (RL模块) | `>=2.2.6` | **不可调和** |
| pandas | `>=0.24` | `>=2.3.2` | 版本跨度大 |
| Python | 3.8-3.12 | 3.12 (推荐) | 需独立环境 |

> **结论**: 同一 Python 运行时安装两者，依赖冲突必然导致稳定性问题。
